...
def load_medallion_pipeline():
    ...
    df_bronze = pd.read_csv(bronze_path) if os.path.exists(bronze_path) else pd.DataFrame()

    # 2. SILVER LAYER
    silver_path = "data/processed/shein_clustered_features.csv"
    df_silver = pd.read_csv(silver_path) if os.path.exists(silver_path) else pd.DataFrame()

    # 3. GOLD LAYER & VENDORS
    gold_path = "data/processed/shein_gold_master.csv"
    df_gold = pd.read_csv(gold_path) if os.path.exists(gold_path) else pd.DataFrame()
    
    vendors_path = "data/processed/vendors_master.csv"
    df_vendors = pd.read_csv(vendors_path) if os.path.exists(vendors_path) else pd.DataFrame()
    ...
    if not df_silver.empty:
        ...
        if "category" in df_silver.columns:
            ...
            df_silver["sub_category"] = df_silver["category"].fillna("Unassigned")
            df_silver["product_type"] = df_silver["product_name"].astype(str).str.split().str[0]

        df_silver["normalized_title"] = df_silver["product_name"].astype(str).str.title().str.strip()
        df_silver["is_missing_metadata"] = df_silver[["price", "category"]].isnull().any(axis=1)
        df_silver["price"] = pd.to_numeric(df_silver["price"], errors="coerce").fillna(25.0)

        # Multi-Level Stock Pipeline Simulation
        df_silver["store_stock"] = np.random.randint(5, 300, size=len(df_silver))
        df_silver["warehouse_stock"] = np.random.randint(20, 800, size=len(df_silver))
        ...
        df_silver["days_of_supply"] = (df_silver["store_stock"] + df_silver["warehouse_stock"]) / df_silver["daily_sales"]

        def assign_po_status(row):
            if row["days_of_supply"] < 15:
                return "PO Issued & In-Transit" if np.random.rand() > 0.4 else "UNRAISED PO (Risk)"
        ...
        df_silver["total_inventory_value"] = (df_silver["store_stock"] + df_silver["warehouse_stock"]) * df_silver["price"]
        df_silver["estimated_cogs"] = df_silver["total_inventory_value"] * 0.60
        df_silver["gross_profit"] = df_silver["total_inventory_value"] - df_silver["estimated_cogs"]
...
try:
    ...
    st.caption("Executive Dashboard: Cleaned Multi-Level Stock Reconciliation, Financial KPIs & Medallion Pipeline")
    st.markdown("---")

    # SIDEBAR CONTROLS
    st.sidebar.header("🎛️ Operations Filter Panel")
    categories = ["All Categories"] + sorted(
        df_silver["sub_category"].dropna().unique().tolist()
    ) if "sub_category" in df_silver.columns else ["All Categories"]
    
    selected_cat = st.sidebar.selectbox("Filter Category", categories)
    audit_only = st.sidebar.checkbox("Show Incomplete Metadata Only", value=False)
    ...
    # TABS NAVIGATION
    tab_overview, tab_reconcile, tab_medallion, tab_catalog, tab_schema = st.tabs(
        [
            "📊 Executive Overview",
    ...
    with tab_overview:
        ...
        dead_stock_value = filtered_df[filtered_df["stock_velocity"] == "Dead Stock"]["total_inventory_value"].sum()
        unraised_risk_value = filtered_df[filtered_df["po_status"] == "UNRAISED PO (Risk)"]["total_inventory_value"].sum() * 0.20
        ...
        f4.metric("💵 Flexible Cash", f"${flexible_cash:,.0f}", help="Dana bebas aman untuk ekspansi/budget baru")
        f5.metric("⚠️ Potential Loss", f"${potential_loss:,.0f}", delta="Risk Capital", delta_color="inverse")
        ...
        health_score = (
            ((len(filtered_df) - filtered_df["is_missing_metadata"].sum()) / len(filtered_df) * 100)
            if len(filtered_df) > 0 else 100
        )
        o2.metric("Catalog Health Score", f"{health_score:.1f}%")
        
        unraised_count = len(filtered_df[filtered_df["po_status"] == "UNRAISED PO (Risk)"])
        o3.metric("Unraised Critical POs", f"{unraised_count:,}", delta="Bottleneck Risk", delta_color="inverse")
        
        in_transit_count = len(filtered_df[filtered_df["po_status"] == "PO Issued & In-Transit"])
        ...
        with col_g1:
            if "price_tier" in filtered_df.columns:
                tier_summary = filtered_df.groupby("price_tier", observed=False)["total_inventory_value"].sum().reset_index()
                fig_pie = px.pie(
                    tier_summary,
                    names="price_tier",
                ...
                st.plotly_chart(apply_fast_style(fig_pie), use_container_width=True)
                st.markdown(
                    """
        ...
        with col_g2:
            if "po_status" in filtered_df.columns:
                po_summary = filtered_df["po_status"].value_counts().reset_index()
                ...
                fig_po_pie.update_layout(xaxis_title="Status PO", yaxis_title="Jumlah SKU")
                st.plotly_chart(apply_fast_style(fig_po_pie), use_container_width=True)
                st.markdown(
                    """
                    <div class="chart-context">
    ...
    with tab_reconcile:
        ...
        st.caption("Perbandingan terpadu antara Stok Toko, Stok Gudang Pusat, Penjualan 30-Hari, dan Kuantitas Order PO.")

        # CLEAN GROUPED BAR CHART WITH TRUNCATED TITLES
        sample_skus = filtered_df.sort_values(by="sales_30d", ascending=False).head(10).copy()
        ...
        fig_reconcile.add_trace(go.Bar(
        ...
        st.plotly_chart(apply_fast_style(fig_reconcile, height=400), use_container_width=True)
        st.markdown(
            """
            <div class="chart-context">
        ...
        with col_r1:
            ...
            st.caption("Kendala operasional pada SKU bernilai *Days of Supply* < 15 hari.")
            unraised_df = filtered_df[filtered_df["po_status"] == "UNRAISED PO (Risk)"]
            if not unraised_df.empty:
                reason_counts = unraised_df["unraised_po_reason"].value_counts().reset_index()
        ...
        with col_r2:
            if not unraised_df.empty:
                ...
                st.plotly_chart(apply_fast_style(fig_reason, height=300), use_container_width=True)

        st.markdown("### 📄 Audit Log Rekonsiliasi Stok & Requisisi Order")
        cols_reconcile_table = [
            "sku_id", "normalized_title", "sub_category", 
            "store_stock", "warehouse_stock", "sales_30d", 
            "days_of_supply", "po_status", "po_quantity", "unraised_po_reason"
        ]
        st.dataframe(
            filtered_df[cols_reconcile_table].sort_values(by="days_of_supply", ascending=True).head(150),
            column_config={
                "sku_id": "SKU ID",
                "normalized_title": "Nama Produk",
                "sub_category": "Kategori",
                "store_stock": st.column_config.NumberColumn("Stok Toko", format="%d"),
                "warehouse_stock": st.column_config.NumberColumn("Stok Gudang", format="%d"),
                "sales_30d": st.column_config.NumberColumn("Sales (30 Hari)", format="%d"),
                "days_of_supply": st.column_config.NumberColumn("Ketahanan (Hari)", format="%.1f Hari"),
                "po_status": "Status PO",
                "po_quantity": st.column_config.NumberColumn("Qty Order PO", format="%d"),
    ...
    with tab_medallion:
        ...
        with col_m1:
            ...
            st.caption("Data mentah dari feed ingestion POS/Catalog tanpa pembersihan.")
        ...
        with col_m2:
            ...
            st.caption("Data terstruktur hasil transformasi, normalisasi, dan klasifikasi.")
            if not df_silver.empty:
                st.metric("Total Cleaned SKUs", f"{len(df_silver):,}")
                cols_silver_preview = [c for c in ["sku_id", "normalized_title", "store_stock", "warehouse_stock", "sales_30d", "po_status"] if c in df_silver.columns]
                st.dataframe(df_silver[cols_silver_preview].head(100), use_container_width=True)
    ...
    with tab_catalog:
        ...
        with col_cat1:
            ...
            if "sub_category" in filtered_df.columns:
                top_cats = filtered_df["sub_category"].value_counts().head(5).reset_index()
        ...
        with col_cat2:
            if "sub_category" in filtered_df.columns:
                fig_cat_bar = px.bar(
                    filtered_df["sub_category"].value_counts().reset_index().head(8),
                    x="count",
                    y="sub_category",
                    orientation="h",
                    title="<b>Volume SKU per Sub-Kategori Utama</b>",
                    color_discrete_sequence=["#818cf8"],
                    labels={"count": "Total SKU", "sub_category": "Sub Kategori"}
                )
                fig_cat_bar.update_layout(yaxis=dict(autorange="reversed"))
                st.plotly_chart(apply_fast_style(fig_cat_bar), use_container_width=True)

        st.markdown("### 📄 Audit Log Normalisasi Katalog (Silver Data)")
        show_cols = [c for c in ["sku_id", "product_name", "normalized_title", "sub_category", "price", "is_missing_metadata"] if c in filtered_df.columns]
        st.dataframe(
    ...
    with tab_schema:
        ...
        with col_s1:
            st.markdown("**🐬 SQL Relational Schema (Multi-Echelon Inventory 3NF)**")
            st.code(
                """
CREATE TABLE inventory_reconciliation (
    ...
    unraised_po_reason VARCHAR(255)
        ...
        with col_s2:
            st.markdown("**🍃 NoSQL BSON / JSON Schema (Supply Chain Tracking)**")
            st.json(
                {
                    "sku_id": "SKU-99218",
                    "inventory_breakdown": {
